...
function DialogClose({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Close>) {
  return (
    <DialogPrimitive.Close
      ...
      className={cn(
        "rounded-xs transition-opacity focus:outline-hidden disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-10  hover:opacity-80 cursor-pointer text-muted-foreground hover:muted-foreground/80",
        className
      )}
      {...props}
    >
...
function DialogHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex justify-between items-start gap-2 text-center sm:text-left', className)}
      {...props}
    />
  )
}
...
function VisuallyHidden({ ...props }: React.ComponentProps<typeof VisuallyHiddenPrimitive.Root>) {
  return <VisuallyHiddenPrimitive.Root data-slot='dialog-visually-hidden' {...props} />
}
